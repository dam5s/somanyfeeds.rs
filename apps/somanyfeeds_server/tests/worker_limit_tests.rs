...
async fn test_worker_run_work_limits_to_20_most_recent_articles() {
    ...
    let mut feed_content = String::from(
...
<rss version="2.0">
    ...
    let mut article_titles: Vec<String> =
        articles.iter().map(|a| a.title.clone().unwrap()).collect();
    article_titles.sort(); // Sort titles to easily check the range

    for i in 6..=25 {
        let expected_title = format!("Article {}", i);
        assert!(
            article_titles.contains(&expected_title),
            "Missing {}",
            expected_title
        );
    }
    ...
    for i in 1..6 {
        ...
        assert!(
            !article_titles.contains(&unexpected_title),
            "Should not have {}",
            unexpected_title
        );
    }
